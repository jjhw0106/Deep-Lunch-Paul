...
# 공원을 나타내는 문자열 배열 park, 로봇 강아지가 수행할 명령이 
# 담긴 문자열 배열 routes가 매개변수로 주어질 때, 
...
#   for idxY, e in enumerate(routes):
#     for idxX in range(int(e[2])):
#       if validate(posY + idxY, posX + idxX, park) == False:
#         continue
#       if e[0] == 'E' or e[0] == 'S':
#         posY += idxY
#         posX += idxX
...
# def validate(posY, posX, park):
#   if 0<= posY < len(park) and 0 <= posX < len(park[0]):
#     if park[posY][posX] == 'X':
#       return False
#   else:
#     return True
...
# solution(["SOO","OXO","OOO"], ["E 2","S 2","W 1"])